                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-400">{ROLE_DEFINITIONS.length} roles defined</p>
                        </div>
                    </div>
            ...
            <main className="p-4">
                        ...
                        {ROLE_DEFINITIONS.map((role) => (
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">Module Access</p>
                                    <div className="flex flex-wrap gap-1">
                                        {moduleKeys
                                            .filter(m => role.permissions[m]?.length > 0)
                                            .map(m => (
                                                <span key={m} className="text-xs px-2 py-1 bg-slate-700 text-slate-300 rounded">
                                                    {MODULES[m]}
                                                </span>
                                            ))}
                                        {moduleKeys.filter(m => role.permissions[m]?.length > 0).length === 0 && (
                                            <span className="text-xs text-slate-500">Dashboard only</span>
                                        )}
                                    </div>
                                </div>
                                ...
                                {selectedRole?.value === role.value && (
                                    <div className="mt-4 pt-4 border-t border-slate-700">
                                        <p className="text-xs text-slate-500 uppercase tracking-wider mb-3">Detailed Permissions</p>
                                        <div className="space-y-2">
                                            {moduleKeys.map(m => (
                                                <div key={m} className="flex items-center justify-between text-sm">
                                                    <span className="text-slate-400">{MODULES[m]}</span>
                                                    {role.permissions[m]?.length > 0 ? (
                                                        getPermissionBadge(role.permissions[m])
                                                    ) : (
                                                        <span className="text-slate-600 text-xs">No access</span>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
                        ...
                        <table className="w-full min-w-[800px]">
                            ...
                            <tbody>
                                {moduleKeys.map(moduleKey => (
                                    <tr key={moduleKey} className="border-b border-slate-700/50 hover:bg-slate-800/50">
                                        <td className="p-4 text-white font-medium">{MODULES[moduleKey]}</td>
                                        {ROLE_DEFINITIONS.map(role => {
                        ...
                        <div className="p-4 border-t border-slate-700">
                            ...
                            <div className="flex flex-wrap gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-slate-400 text-sm">Create</span>
                                </div>
                                <div className="flex items-center gap-2">
                ...
                <div className="glass-card p-6 mt-4">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h3 className="text-white font-semibold mb-1">About Role-Based Access Control</h3>
