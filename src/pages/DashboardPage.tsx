...
export default function DashboardPage() {
    ...
    const inProgressBatchCount =
        todayBatches?.filter((b) => b.status === 'IN_PROGRESS' || b.status === 'COOLING').length ?? 0;
    ...
    const getRoleBasedModules = () => {
        ...
        return allModules.filter((module) => module.roles.includes(userData.role));
    };
    ...
    return (
        <div className="relative overflow-hidden">
            ...
            <main className="p-4 relative z-10">
                ...
                {statsError && (
                    <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 mb-4">
                        <p className="text-red-400 text-sm">
                            Failed to load some dashboard stats. Data shown may be stale.
                        </p>
                    </div>
                )}
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-start justify-between mb-3">
                                    <div
                                        className={`w-11 h-11 bg-gradient-to-br ${module.color} rounded-xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:shadow-xl transition-all duration-300`}
                                    >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <svg
                                            className="w-4 h-4 text-foreground-faint group-hover:text-foreground-muted group-hover:translate-x-0.5 transition-all duration-300"
                                            fill="none"
                                            viewBox="0 0 24 24"
                                            stroke="currentColor"
                                        >
                                ...
                                <h4 className="text-foreground font-semibold mb-0.5 group-hover:text-blue-100 transition-colors">
                                    {module.name}
                                </h4>
                ...
                {/* Maintenance Alerts */}
                {((assetStats && (assetStats.breakdownAssets > 0 || assetStats.pmDue > 0)) ||
                    ...
                    <div className="mt-6 mb-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {assetStats && assetStats.breakdownAssets > 0 && (
                                <div
                                    className="glass-card p-4 border border-red-500/30 cursor-pointer hover:bg-surface-hover transition-colors"
                                    onClick={() => navigate('/assets')}
                                >
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                                        <p className="text-red-400 font-medium text-sm">
                                            {assetStats.breakdownAssets} Asset
                                            {assetStats.breakdownAssets > 1 ? 's' : ''} Down
                                        </p>
                                    </div>
                            ...
                            {jobStats && jobStats.criticalJobs > 0 && (
                                <div
                                    className="glass-card p-4 border border-orange-500/30 cursor-pointer hover:bg-surface-hover transition-colors"
                                    onClick={() => navigate('/maintenance')}
                                >
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-orange-500" />
                                        <p className="text-orange-400 font-medium text-sm">
                                            {jobStats.criticalJobs} Critical Job{jobStats.criticalJobs > 1 ? 's' : ''}
                                        </p>
                                    </div>
                                    <p className="text-foreground-faint text-xs mt-1">
                                        Open critical maintenance tasks
                                    </p>
                                </div>
                            )}
                            {assetStats && assetStats.pmDue > 0 && (
                                <div
                                    className="glass-card p-4 border border-yellow-500/30 cursor-pointer hover:bg-surface-hover transition-colors"
                                    onClick={() => navigate('/assets')}
                                >
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-yellow-500" />
                                        <p className="text-yellow-400 font-medium text-sm">
                                            {assetStats.pmDue} PM Overdue
                                        </p>
                                    </div>
                ...
                <div className="gradient-border-card p-5 mt-8">
                    <div className="flex items-center justify-between">
                            ...
                            <div>
                                ...
                                {activeShift ? (
                                    <p className="text-foreground-muted text-sm">
                                        {SHIFT_TYPES.find((s) => s.value === activeShift.shiftType)?.label ||
                                            `Shift ${activeShift.shiftType}`}{' '}
                                        &bull; {SHIFT_TYPES.find((s) => s.value === activeShift.shiftType)?.time || ''}
                                    </p>
                                ) : (
                        ...
                        <div className="text-right">
                            {activeShift ? (
                                <>
                                    <p className="text-xs text-foreground-faint uppercase tracking-wider mb-0.5">
                                        Started
                                    </p>
