...
export interface CompleteStepData {
    ...
    temperature?: number;
    pressure?: number;
    inputWeight?: number;                    // For LOADING step (step 3)
    nitrogenPurged?: boolean;               // For VENTING/CARBON_DISCHARGE (steps 10, 11)
    pyrolysisReadings?: PyrolysisReadingData[];  // For PYROLYSIS step (step 8)
    gateEntryIds?: string[];                // For LOADING step - linked gate entries
}
...
export interface RecordOutputData {
    ...
    qualityGrade?: string;
    photoUrl?: string;
    inventoryItemId?: string;  // Link to inventory item for automatic receipt
}
...
export async function recordOutput(
    ...
    if (outputData.inventoryItemId) {
        const { receiptFromBatch } = await import('./inventoryService');
