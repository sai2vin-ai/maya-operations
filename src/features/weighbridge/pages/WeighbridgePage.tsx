...
export default function WeighbridgePage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            default: return 'bg-slate-500/20 text-foreground-muted border-slate-500';
        }
    };
    ...
    const getTypeBadge = (type: string) => {
        switch (type) {
            ...
            default: return 'bg-slate-500/20 text-foreground-muted';
        }
    };
                    ...
                    <>
                                    ...
                                    {pendingEntries.map((entry: WeighbridgeEntry) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="text-foreground font-medium">{entry.vehicleNumber}</span>
                                                    <span className="text-foreground-faint text-sm ml-2">{entry.entryNumber}</span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-2 mb-4">
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg ${filter === 'all' ? 'bg-blue-500 text-foreground' : 'bg-surface-tertiary text-foreground-secondary'}`}
                            >
                                All
                            </button>
                            <button
                                onClick={() => setFilter('RM_IN')}
                                className={`px-4 py-2 rounded-lg ${filter === 'RM_IN' ? 'bg-cyan-500 text-foreground' : 'bg-surface-tertiary text-foreground-secondary'}`}
                            >
                                RM IN
                            </button>
                            <button
                                onClick={() => setFilter('FG_OUT')}
                                className={`px-4 py-2 rounded-lg ${filter === 'FG_OUT' ? 'bg-orange-500 text-foreground' : 'bg-surface-tertiary text-foreground-secondary'}`}
                            >
                                FG OUT
                            </button>
                        </div>
                        ...
                        <div className="glass-card p-6">
                            <h2 className="text-lg font-semibold text-foreground mb-4">Today's Entries</h2>
                            {filteredEntries.length > 0 ? (
                                <div className="overflow-x-auto">
                                    <table className="w-full">
                                        <thead className="bg-surface-tertiary/50">
                                            <tr>
                                                <th className="text-left p-3 text-foreground-secondary font-medium">Entry #</th>
                                                <th className="text-left p-3 text-foreground-secondary font-medium">Type</th>
                                                <th className="text-left p-3 text-foreground-secondary font-medium">Vehicle</th>
                                                <th className="text-left p-3 text-foreground-secondary font-medium">Material</th>
                                                <th className="text-right p-3 text-foreground-secondary font-medium">Net Weight</th>
                                                <th className="text-left p-3 text-foreground-secondary font-medium">Time</th>
                                                <th className="text-left p-3 text-foreground-secondary font-medium">Status</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-700">
                                            {filteredEntries.map((entry: WeighbridgeEntry) => (
                                                <tr
                                                    ...
                                                    onClick={() => navigate(`/weighbridge/${entry.id}`)}
                                                >
                                                    <td className="p-3 text-foreground font-mono">{entry.entryNumber}</td>
                                                    <td className="p-3">
                                                        <span className={`px-2 py-1 rounded text-xs ${getTypeBadge(entry.entryType)}`}>
                                                            {entry.entryType === 'RM_IN' ? '📥 IN' : '📤 OUT'}
                                                        </span>
                                                    ...
                                                    <td className="p-3 text-foreground-secondary">{entry.materialName || '-'}</td>
                                                    <td className="p-3 text-right text-foreground font-medium">
                                                        {entry.netWeight ? `${entry.netWeight.toLocaleString()} KG` : '-'}
                                                    </td>
                                                    <td className="p-3 text-foreground-muted">{formatDate(entry.createdAt)}</td>
                                                    <td className="p-3">
                                                        <span className={`px-2 py-1 rounded-full border text-xs ${getStatusBadge(entry.status)}`}>
                                                            {entry.status}
                                                        </span>
                                ...
                                <p className="text-foreground-faint text-center py-8">No entries today. Create one to get started!</p>
                            )}
                        </div>
                    </>
                )}
